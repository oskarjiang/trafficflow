...
const inputFilePath = path.join(__dirname, "..", "gtfs_vt", "shapes.txt");
const outputDir = path.join(__dirname, "..", "public", "gtfs_vt", "segments");
...
async function splitFile(totalLines) {
  ...
  let outputStream = fs.createWriteStream(
    path.join(outputDir, `shapes_part${currentSegment}.txt`)
  );
...
(async () => {
  try {
    console.log("Counting lines in shapes.txt...");
